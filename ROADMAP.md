...
### Medium — ✅ shipped
- ✅ **Live size + meta inspector** on each loader's detail page (slug,
  duration, iteration, lit cells, byte count, easing).
- ✅ **Color recoloring picker** in the toolbar; `DotMatrixIcon` now defaults
  to `currentColor` so the gallery wrapper recolors every loader at once.
- ✅ **Speed scrubber** (0.25× → 2×) drives a `speedMultiplier` prop that
  scales both `durationMs` and per-cell delays, so the timing stays coherent
  at any speed.
- ✅ **Per-loader detail page** at `/icon/$slug` — large preview, copyable
  React snippet, copyable standalone SVG, "used at typical sizes" strip.
- ✅ **"Used in" strip** on each detail page at 16 / 24 / 32 / 48 / 72 px.
